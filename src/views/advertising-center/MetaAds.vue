<template>
  <section class="page-shell meta-ads-page">
    <div v-show="summaryCardsVisible" v-loading="summaryLoading" class="meta-ads-metric-grid">
      <article
        v-for="card in summaryCards"
        :key="card.key"
        class="meta-ads-metric-card"
        :data-kind="card.kind"
      >
        <div class="meta-ads-metric-card__header">
          <span>{{ card.label }}</span>
          <component :is="card.icon" class="meta-ads-metric-card__icon" />
        </div>
    ...
    <div class="work-panel meta-ads-list-panel">
      <div class="meta-ads-panel-heading">
        <div>
          <p class="page-kicker">Meta Ads</p>
          <h2>广告系列数据</h2>
        </div>
        <el-button
          text
      ...
      <el-row :gutter="12" class="search-row meta-ads-search-row">
        ...
        <el-col :xs="24" :sm="12" :md="8" :lg="4">
          <el-select v-model="queryForm.effectiveStatus" placeholder="请选择广告状态" clearable>
            <el-option
        ...
        <el-col :xs="24" :sm="24" :md="24" :lg="24" class="meta-ads-search-actions">
          <el-button type="success" @click="handleOpenSpecifiedProductDialog">指定产品</el-button>
          <el-button type="primary" :icon="Search" @click="handleSearch">查询</el-button>
          <el-button :icon="RefreshLeft" @click="handleReset">重置</el-button>
          <el-button type="warning" :disabled="tableLoading" @click="handleOpenAllocatePersonDialog">分配投手</el-button>
          <el-button type="success" plain :disabled="tableLoading" @click="handleOpenLinkProductDialog">绑定产品</el-button>
          <el-tag
            v-if="specifiedProduct"
            type="success"
            effect="plain"
            closable
            class="meta-ads-specified-product"
      ...
      <div class="meta-ads-table-scroll">
        <el-table
          ref="tableRef"
          v-loading="tableLoading"
          :data="campaignList"
          row-key="uid"
          class="basic-table meta-ads-table"
          height="100%"
          border
          stripe
          empty-text="暂无 Meta 广告系列数据"
        ...
        >
          ...
          >
            <template #default="{ row }">{{ formatMetaAdsCell(row.campaignName) }}</template>
          </el-table-column>
          ...
          >
            <template #default="{ row }">
              {{ formatMetaAdsCell(row.adsAccountName) }}
            </template>
          </el-table-column>

          <el-table-column prop="adsAccountStatus" label="账户状态" min-width="110" align="center">
            <template #default="{ row }">
              <el-tag
                v-if="getMetaAdsAccountStatusOption(row.adsAccountStatus)"
                :type="getMetaAdsAccountStatusOption(row.adsAccountStatus).tagType"
                effect="plain"
              >
                {{ getMetaAdsAccountStatusOption(row.adsAccountStatus).label }}
              </el-tag>
              <span v-else>{{ formatMetaAdsCell(row.adsAccountStatus) }}</span>
            </template>
          </el-table-column>

          <el-table-column prop="effectiveStatus" label="状态" min-width="100" align="center">
            <template #default="{ row }">
              <el-tag
                v-if="getMetaAdsStatusOption(row.effectiveStatus)"
                :type="getMetaAdsStatusOption(row.effectiveStatus).tagType"
                effect="plain"
              >
                {{ getMetaAdsStatusOption(row.effectiveStatus).label }}
              </el-tag>
              <span v-else>{{ formatMetaAdsCell(row.effectiveStatus) }}</span>
            </template>
          </el-table-column>

          <el-table-column prop="costAmount" label="广告花费" min-width="140" align="right">
            <template #default="{ row }">
              {{ formatMetaAdsAmount(row.costAmount, row.currencyCode) }}
            </template>
          </el-table-column>

          <el-table-column prop="impressions" label="展示次数" min-width="120" align="right">
            <template #default="{ row }">{{ formatMetaAdsCount(row.impressions) }}</template>
          </el-table-column>

          <el-table-column prop="clicks" label="点击次数" min-width="110" align="right">
            <template #default="{ row }">{{ formatMetaAdsCount(row.clicks) }}</template>
          </el-table-column>

          <el-table-column prop="conversions" label="转化次数" min-width="120" align="right">
            <template #default="{ row }">{{ formatMetaAdsCount(row.conversions, 2) }}</template>
          </el-table-column>
          ...
          >
            <template #default="{ row }">
              {{ formatMetaAdsAmount(row.conversionsValue, row.currencyCode) }}
            </template>
          </el-table-column>

          <el-table-column prop="ctr" label="点击率" min-width="100" align="right">
            <template #default="{ row }">{{ formatMetaAdsRate(row.ctr) }}</template>
          </el-table-column>

          <el-table-column prop="cpc" label="单次点击费用" min-width="135" align="right">
            <template #default="{ row }">
              {{ formatMetaAdsAmount(row.cpc, row.currencyCode) }}
            </template>
          </el-table-column>

          <el-table-column prop="cvr" label="转化率" min-width="100" align="right">
            <template #default="{ row }">{{ formatMetaAdsRate(row.cvr) }}</template>
          </el-table-column>

          <el-table-column prop="roas" label="回报率" min-width="100" align="right">
            <template #default="{ row }">{{ formatMetaAdsRoas(row.roas) }}</template>
          </el-table-column>

          <el-table-column prop="cpa" label="单次行动成本" min-width="135" align="right">
            <template #default="{ row }">
              {{ formatMetaAdsAmount(row.cpa, row.currencyCode) }}
            </template>
          </el-table-column>

          <el-table-column prop="startTime" label="投放开始时间" min-width="175">
            <template #default="{ row }">{{ formatMetaAdsCell(row.startTime) }}</template>
          </el-table-column>

          <el-table-column prop="stopTime" label="投放结束时间" min-width="175">
            <template #default="{ row }">{{ formatMetaAdsCell(row.stopTime) }}</template>
          </el-table-column>
          ...
          <el-table-column label="绑定产品" min-width="390">
            <template #default="{ row }">
              <div
                ...
                class="meta-ads-bound-product"
              >
                <el-image
                  v-if="row.crossPlatformProductCommonInfoVO.uid"
                  class="meta-ads-bound-product__image"
                  :src="buildCrossPlatformProductImageUrl(row.crossPlatformProductCommonInfoVO.uid)"
                  fit="cover"
                  lazy
                >
                  <template #error>
                    <div class="meta-ads-bound-product__image-fallback">暂无图片</div>
                  </template>
                </el-image>
                <div v-else class="meta-ads-bound-product__image-fallback">暂无图片</div>

                <div class="meta-ads-bound-product__details">
                  <div class="meta-ads-bound-product__field">
                    <span>平台</span>
                    <b>
                      {{
          ...
          >
            <template #default="{ row }">
              {{ findMetaAdsPersonName(personOptions, row.personInCharge) }}
            </template>
          </el-table-column>
        </el-table>
      </div>

      <div class="meta-ads-pagination">
        <el-pagination
...
import MetaAdsAllocatePersonDialog from '@/components/meta-ads/MetaAdsAllocatePersonDialog.vue'
...
import {
  aggregateMetaAdsDailyPerformance,
  buildMetaAdsCommonParams,
  buildMetaAdsListParams,
  findMetaAdsPersonName,
  formatMetaAdsAmount,
  formatMetaAdsCell,
  formatMetaAdsCount,
  formatMetaAdsRate,
  formatMetaAdsRoas,
  getMetaAdsStatusOption,
  getMetaAdsAccountStatusOption,
  META_ADS_CAMPAIGN_STATUS_OPTIONS,
} from '@/utils/MetaAds'
...
// 翻页和操作后的刷新沿用已提交条件，避免使用尚未点击查询的输入。
const appliedQuery = ref({ ...queryForm, dateRange: [...queryForm.dateRange] })
...
const statusOptions = META_ADS_CAMPAIGN_STATUS_OPTIONS
const crossPlatformProductOptions = CROSS_PLATFORM_PRODUCT_PLATFORM_OPTIONS

const selectedDateSummary = computed(() =>
  Array.isArray(appliedQuery.value.dateRange) && appliedQuery.value.dateRange.length === 2
    ? `${appliedQuery.value.dateRange[0]} 至 ${appliedQuery.value.dateRange[1]}`
    : '未选择日期',
)
const specifiedProductSummary = computed(
  () => specifiedProduct.value?.productTitle || specifiedProduct.value?.uid || '-',
)
const summary = computed(() => aggregateMetaAdsDailyPerformance(dailyPerformance.value))
const summaryCards = computed(() => [
  {
    key: 'costAmount',
    label: '总广告花费',
    value: formatMetaAdsAmount(summary.value.costAmount),
    icon: markRaw(Money),
    kind: 'blue',
  },
  {
    ...
    label: '总转化价值',
    value: formatMetaAdsAmount(summary.value.conversionsValue),
  ...
  },
  {
    key: 'impressions',
    label: '总展示次数',
    value: formatMetaAdsCount(summary.value.impressions),
    icon: markRaw(View),
    kind: 'violet',
  },
  {
    key: 'clicks',
    label: '总点击次数',
    value: formatMetaAdsCount(summary.value.clicks),
    icon: markRaw(Mouse),
    kind: 'orange',
  },
  {
    key: 'conversions',
    label: '总转化次数',
    value: formatMetaAdsCount(summary.value.conversions, 2),
...
const loadCampaignList = async () => {
  const params = buildMetaAdsListParams(appliedQuery.value, pagination)
  ...
  const response = await Request({
    url: '/metaAds/loadMetaAdsCampaignInsights',
    method: 'post',
    params,
    showLoading: false,
  }).finally(() => {
...
const loadDailyPerformance = async () => {
  const params = buildMetaAdsCommonParams(appliedQuery.value)
  if (!params) return null

  const requestId = ++summaryRequestId
  summaryLoading.value = true
  const response = await Request({
    url: '/metaAds/getMetaAdsCampaignInsightsDailyPerformance',
    method: 'post',
    params,
    showLoading: false,
  }).finally(() => {
...
const refreshPageData = () => Promise.all([loadCampaignList(), loadDailyPerformance()])

const loadPageData = () => {
  if (!buildMetaAdsCommonParams(queryForm)) {
    Message.warning('请选择日期范围')
    return Promise.resolve([])
  }
  appliedQuery.value = { ...queryForm, dateRange: [...queryForm.dateRange] }
...
const handleSelectionChange = (rows) => {
  selectedCampaigns.value = Array.isArray(rows)
    ? rows.filter((campaign) => campaign?.uid)
    : []
}
...
const submitProductBinding = async (newMappingProductUid, successMessage, submittingState) => {
  // Meta 映射表的 adsCampaignId 存储 uid（campaignId + adsAccountId）。
  const adsCampaignIdList = linkCampaigns.value
    .map((campaign) => String(campaign?.uid ?? '').trim())
    .filter(Boolean)
  if (
    !newMappingProductUid ||
    adsCampaignIdList.length === 0 ||
    linkProductSubmitting.value ||
    cancelLinkProductSubmitting.value
  ) {
    return null
  }
